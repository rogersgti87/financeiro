...
class Invoice extends Model
{
    ...
    public static function generateBilletPayment($invoice_id){
        ...
        $invoice = DB::table('invoices as i')
                    ->select('i.id','c.email','c.name','c.document','c.phone','c.address','c.number','c.complement',
    ...
    public static function generatePixPayment($invoice_id){
      ...
      $invoice = DB::table('invoices as i')
      ->select('i.id','c.email','c.name','c.document','c.phone','c.address','c.number','c.complement','c.district','c.city','c.state','c.cep','s.id as service_id','s.name as service_name','s.price as service_price')
      ->join('customer_services as cs','i.customer_service_id','cs.id')
      ->join('customers as c','cs.customer_id','c.id')
      ->join('services as s','cs.service_id','s.id')
      ->where('i.id',$invoice_id)
